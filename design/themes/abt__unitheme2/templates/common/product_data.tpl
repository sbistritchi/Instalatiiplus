...
{hook name="products:product_name"}
    ...
    {elseif $show_trunc_name}
        {if $hide_links}<strong>{else}<a href="{"products.view?product_id=`$product.product_id`"|fn_url}" class="product-title" title="{$product.product|strip_tags}" {live_edit name="product:product:{$product.product_id}" phrase=$product.product}>{/if}{if $show_labels_in_title}{hook name="products:dotd_product_label"}{/hook}{/if}{$product.product|truncate:90:"...":true nofilter}{if $hide_links}</strong>{else}</a>{/if}
    {/if}
{/hook}
{/capture}
{if $no_capture}
...
{capture name="buttons_product"}
    ...
    {hook name="products:add_to_cart"}
        {if $settings.abt__ut2.product_list.product_variations.allow_variations_selection[$settings.ab__device] === "YesNo::YES"|enum && $product.has_options && !$show_product_options && !$details_page && $force_show_add_to_cart_button != "Y"}
            {$preselected_options = $product.combination}
            {if $settings.ab__device === "mobile"}
                <span class="ty-btn ut2-btn__options ty-btn__primary ty-btn__add-to-cart cm-ab-load-select-variation-content" data-ca-product-id="{$product.product_id}"{if $preselected_options} data-ca-combination="{$preselected_options}"{/if}><span class="ty-icon ut2-icon-use_icon_cart"></span><bdi>{__("add_to_cart")}</bdi></span>
            {else}
