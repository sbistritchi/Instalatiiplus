...
<div id="cart_items">
    ...
    <thead>
        <tr>
            ...
            <th class="ty-cart-content__title ty-right quantity-cell">{__("quantity")}</th>
        ...
        {foreach from=$cart_products item="product" key="key" name="cart_products"}
            ...
            {hook name="checkout:items_list"}
                ...
                {if !$cart.products.$key.extra.parent}
                    <tr>
                        <td class="ty-cart-content__product-elem ty-cart-content__image-block" style="width: calc({$settings.Thumbnails.product_cart_thumbnail_width}px + (var(--gap-sm) * 2))">
                        ...
                        <td class="ty-cart-content__product-elem ty-cart-content__description">
                            <div class="abt__ut2--content__description__in">
                                {strip}
                                    <a href="{"products.view?product_id=`$product.product_id`"|fn_url}"
                                    ...
                                    {if !$product.exclude_from_calculate}
                                        <a class="{$ajax_class} ty-cart-content__product-delete ty-delete-big"
                                           ...
                                           title="{__("remove")}">&nbsp;{include_ext file="common/icon.tpl" class="ty-icon-cancel-circle ty-delete-big__icon"}</a>
                                    {/if}
                                {/strip}
                                            ...
                                            <div class="ty-group-block hidden" id="discount_{$key}"
                                                ...
                                                <table class="ty-cart-content__more-info ty-table">
                                                    ...
                                                    <tr>
                                                        ...
                                                        {if $product.discount|floatval}
                                                            <th class="ty-cart-content__more-info-title">{__("discount")}</th>{/if}
                                                        {if $product.taxes && $settings.Checkout.tax_calculation != "subtotal"}
                                                            <th>{__("tax")}</th>{/if}
                        ...
                        <td class="ty-cart-content__product-elem ty-cart-content__price cm-reload-{$obj_id}" id="price_subtotal_update_{$obj_id}">
                            <strong>{include file="common/price.tpl" value=$product.display_subtotal span_id="product_subtotal_`$key`" class="price"}</strong>
