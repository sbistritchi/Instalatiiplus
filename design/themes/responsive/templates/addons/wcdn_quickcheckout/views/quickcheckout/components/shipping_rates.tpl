            ...
            {if $group.shippings && !$group.all_edp_free_shipping && !$group.shipping_no_required}
                ...
                {foreach from=$group.shippings item="shipping"}
                    ...
                    {hook name="checkout:shipping_method"}
                        {if $display == "radio"}
                            <div class="ty-shipping-options__method ajax-change {if $cart.chosen_shipping.$group_key == $shipping.shipping_id} selected {/if}">
                                <div class="form-check">
                                    <input type="radio" class="ty-valign form-check-input" id="sh_{$group_key}_{$shipping.shipping_id}" name="shipping_ids[{$group_key}]" value="{$shipping.shipping_id}" onclick="fn_calculate_total_shipping_cost_quickcheckout();" {$checked} />
                                    <label for="sh_{$group_key}_{$shipping.shipping_id}" class="ty-valign form-check-label">{$shipping.shipping} {$delivery_time}{if $rate}: {$rate nofilter}{/if}</label>
                                </div>
                            </div>

                        {elseif $display == "select"}
                            <option value="{$shipping.shipping_id}" {$selected}>{$shipping.shipping} {$delivery_time}{if $rate} - {$rate nofilter}{/if}</option>

                        {elseif $display == "show"}
                            <p>
                                {$strong_begin}{$rate.name} {$delivery_time}{if $rate} - {$rate nofilter}{/if}{$strong_begin}
                            </p>
                        {/if}
