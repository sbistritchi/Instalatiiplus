<div>
    <form class="cm-ajax cm-ajax-force cm-ajax-full-render" name="coupon_code_form{$position}" id="promotion_coupon_form" action="{""|fn_url}" method="post">
        ...
        {hook name="checkout:discount_coupons"}
			<label for="coupon_field{$position}">{__("promo_code")}</label>
...
{$_redirect_url = $smarty.request.dispatch}
    ...
    {capture name="promotion_info"} 
        {hook name="checkout:applied_coupons_items"}
            ...
            <li class="ty-coupons__item">
                {__("coupon")} "{$coupon_code}"  <a class="btn coupons-delete cm-ajax cm-ajax-full-render" style="margin-top:-2px" href="{"checkout.delete_coupon?coupon_code=`$coupon_code`&redirect_url=`$_redirect_url`"|fn_url}" data-ca-target-id="checkout*,cart_status*,cart_items"><i class="fa fa-times"></i></a>
