    ...
    {if $profile_fields.$section}
        {if $address_flag}
            <div class="form-group" style="width:100%">
					...
					<label for="sw_{$body_id}_suffix_no">
						<input type="hidden" name="ship_to_another" value="0" id="sw_{$body_id}_suffix_yes" checked />
						<input class="cm-switch-availability cm-switch-visibility form-check-input" type="checkbox" name="ship_to_another" value="1" id="sw_{$body_id}_suffix_no" {if $ship_to_another}checked="checked"{/if} />
						Vreau factura pe firma
					</label>
        ...
        <div class="form-group {$field.class}">
            ...
            {elseif $field.field_type == "R"}  {* Radiogroup *}
                <div id="{$id_prefix}elm_{$field.field_id}" class="d-flex">
                    {foreach from=$field.values key=k item=v name="rfe"}
                    <div class="form-check ptype">
                        <input class="radio form-check-input {if !$skip_field}{$_class}{else}cm-skip-avail-switch{/if} {$id_prefix}elm_{$field.field_id}" type="radio" id="{$id_prefix}elm_{$field.field_id}_{$k}" name="{$data_name}[{$data_id}]" value="{$k}" {if (!$value && $smarty.foreach.rfe.first) || $value == $k}checked="checked"{/if} {if !$skip_field}{$disabled_param nofilter}{/if} />
                        <label class="radio form-check-label" for="{$id_prefix}elm_{$field.field_id}_{$k}">{$v}</label>
                    </div>
                    {/foreach}
                </div>
