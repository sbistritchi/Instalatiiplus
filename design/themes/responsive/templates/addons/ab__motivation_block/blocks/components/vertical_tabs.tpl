...
{foreach $ab__motivation_items as $ab__mb_item}
    ...
    {if trim(strip_tags($smarty.capture.$capture_name, '<img>'))}
        <div class="ab__mb_item" style="background-color: {$addons.ab__motivation_block.bg_color}{if $addons.ab__motivation_block.bg_color !="#ffffff"};border-color: {$addons.ab__motivation_block.bg_color}{/if}">
            {$expanded = $ab__mb_item["expanded_`$settings.ab__device`"]|default:$ab__mb_item.expanded_desktop}
            <div id="sw_{$key}" class="ab__mb_item-title cm-combination{if $addons.ab__motivation_block.save_element_state == "YesNo::YES"|enum} ab-mb-save-state{/if}{if ($expanded == "YesNo::YES"|enum && $smarty.cookies.$key != "YesNo::NO"|enum) || $smarty.cookies.$key == "YesNo::YES"|enum} open{/if}">
