...
if (!empty($conditions) && $status == ObjectStatuses::ACTIVE && !$this->checkVersion($version, $conditions)) {
$result = false;
$min = $max = '';
...
function fn_settings_actions_addons_ab__stickers(&$a, $b)
{
$a == ObjectStatuses::ACTIVE && !(new ABXmlScheme(substr(__FUNCTION__, 27)))->checkDependencies() && $a = ObjectStatuses::DISABLED;
}
function fn_settings_variants_addons_ab__stickers_ts_appearance()
{
return fn_ab__stickers_sticker_get_ts_appearance_styles();
}
function fn_settings_variants_addons_ab__stickers_p_appearance()
{
return fn_ab__stickers_sticker_get_ts_appearance_styles();
}