...
final class Drawer implements AlphaBlendingAwareDrawerInterface, InfoProvider
{
     ...
     */
    public function text($string, AbstractFont $font, PointInterface $position, $angle = 0, $width = null, $params = array())
    {
        ...
        // AB: fix text size and set text stroke
        $text_color = $this->getColor($font->getColor());
        $h = imagesy($this->resource);

        $x += round(($fontsize * 15 / 100) / 2 + (isset($params['stroke_width']) ? $params['stroke_width'] / 2 : 0));
        $y += round(($h * 15 / 100) / 2 + (isset($params['stroke_width']) ? $params['stroke_width'] / 2 : 0));

        if (isset($params['stroke_width'])) {
            $text_stroke_color = $this->getColor($params['stroke_color']);

            if ($this->setTextStroke($this->resource, $fontsize, $angle, $x, $y, $text_color, $text_stroke_color, $fontfile, $string, (float)$params['stroke_width']) === false) {
                $this->revertAlphaBlending();
                throw new RuntimeException('Font mask operation failed');
            }
        } elseif (imagefttext($this->resource, $fontsize, $angle, $x, $y, $text_color, $fontfile, $string) === false) {
            $this->revertAlphaBlending();
            throw new RuntimeException('Font mask operation failed');
        }
    ...
    private function setTextStroke(&$image, $size, $angle, $x, $y, &$textcolor, &$strokecolor, $fontfile, $text, $px)
    {
