...
if (!empty($conditions) && $status == \Tygh\Enum\ObjectStatuses::ACTIVE && !$this->checkVersion($version, $conditions)) {
$result = false;
$min = $max = '';
...
fn_set_notification(\Tygh\Enum\NotificationSeverity::ERROR, __('error'), $msg);
}
}
}
return $result;
...
function fn_settings_actions_addons_ab__motivation_block(&$a, $b)
{
$a == \Tygh\Enum\ObjectStatuses::ACTIVE && !(new ABXmlScheme(substr(__FUNCTION__, 27)))->checkDependencies() && $a = \Tygh\Enum\ObjectStatuses::DISABLED;
}
