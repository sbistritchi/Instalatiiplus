...
return $feature_id ? $feature_id : db_get_field("SELECT feature_id FROM ?:product_features WHERE status = ?s AND feature_type = ?s AND company_id = ?i", ObjectStatuses::ACTIVE, ProductFeatures::EXTENDED, fn_get_runtime_company_id());
}
