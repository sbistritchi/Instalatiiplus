...
function fn_store_profile_fields($profile_data, $object_id, $object_type)
{
    ...
    $sanitize = static function ($array) {
        $fn = static function ($item) use (&$fn) {
            if (is_array($item)) {
                return array_map($fn, $item);
            }

            if (is_string($item)) {
                return SecurityHelper::sanitizeHtml($item);
            }
    ...
    $profile_data['fields'] = $sanitize($profile_data['fields']);
    ...
    foreach ($current_fields_values as $field_id => $field_value) {
        if (isset($profile_data['fields'][$field_id]) && $profile_data['fields'][$field_id] !== '') {
            continue;
        }
...
function fn_auth_routines($request, $auth)
{
    ...
    if (
        !empty($user_data)
        && (!fn_check_user_type_admin_area($user_data) && SiteArea::isAdmin(AREA) || !fn_check_user_type_access_rules($user_data))
    ) {
        fn_set_notification(NotificationSeverity::ERROR, __('error'), __('error_area_access_denied'));
        $status = false;
    }
    ...
    return [$status, $user_data, $user_login, $password, $salt];
}
 ...
 *
 * @return bool|string|int[]|null Array or string with user data or false on error
 */
function fn_recover_password_login($ekey = null, $get_user_data = false)
{
    ...
    $user_info = !empty($user_id) ? fn_get_user_short_info($user_id) : [];
    ...
    if (!$is_user_on_correct_site_area) {
        fn_set_notification(NotificationSeverity::ERROR, __('error'), __('text_ekey_not_valid'));
        return false;
    }
    ...
    if ($user_status === LOGIN_STATUS_OK) {
        fn_set_notification(NotificationSeverity::NOTICE, __('notice'), __('text_change_password'), 'I', 'notice_text_change_password');
        if (!$get_user_data) {
            $result = $user_id;
        }
    } else {
    ...
    if ($get_user_data) {
        $result = [(int) $user_id, (int) $user_status];
    }
 ...
 *
 * @return array<string, int|string>|bool Array with user data and ekey or false on error
 *
 * @psalm-return array{
 *     company_id: int,
 *     key: string,
...
function fn_recover_password_generate_key($user_email, $notify = true, $company_id = '', $area = AREA)
{
    ...
    /** @var int $user_id */
    $user_id = db_get_field('SELECT user_id FROM ?:users WHERE email = ?s ?p', $user_email, $condition);
    if (empty($user_id)) {
        fn_set_notification(NotificationSeverity::ERROR, __('error'), __('error_login_not_exists'));
        return false;
    }
    ...
    if (isset($user_data['status']) && $user_data['status'] === ObjectStatuses::DISABLED) {
        fn_set_notification(NotificationSeverity::ERROR, __('error'), __('error_account_disabled'));
        return false;
    }

    $count_ekeys = fn_recovery_password_get_ekeys_count($user_id);
    if ($count_ekeys >= RECOVERY_PASSWORD_EKEYS_LIMIT) {
        fn_set_notification(NotificationSeverity::WARNING, __('warning'), __('error_count_recover_password_ekeys'));
        return false;
    }

    $ekey = fn_generate_ekey($user_data['user_id'], RECOVERY_PASSWORD_EKEY_TYPE, RECOVERY_PASSWORD_TTL);
    if ($notify) {
        /** @var \Tygh\Notifications\EventDispatcher $event_dispatcher */
        $event_dispatcher = Tygh::$app['event.dispatcher'];
        /** @var \Tygh\Storefront\Storefront $storefront */
        ...
        $result = [
            ...
            'key' => $ekey,
            'user_type' => (string) $user_data['user_type']
        ];
    }
...
/**
 * Get count of generated ekeys for user.
 *
 * @param int $user_id User ID
 *
 * @return int|null Count of generated ekeys for recover password for user
 */
function fn_recovery_password_get_ekeys_count($user_id = 0)
{
    if (!$user_id) {
        return null;
    }
    ...
    $ekeys = fn_get_ekeys([
        'object_id' => $user_id,
        'object_type' => RECOVERY_PASSWORD_EKEY_TYPE
    ]);
...
/**
 * Checks is user can log in as another user.
 *
 * @param array{user_type: string, is_root: string} $current_user_info Current user info
 * @param array{user_type: string, is_root: string} $target_user_info  Target user info
 */
function fn_is_user_can_login_as(array $current_user_info, array $target_user_info): bool
{
    ...
    $login_as_map = [
        ...
        UserTypes::ADMIN      => [USER_TYPE_ADMIN_ROOT => false, UserTypes::ADMIN => true,  USER_TYPE_VENDOR_ROOT => true,  UserTypes::VENDOR => true,  UserTypes::CUSTOMER => true],
        USER_TYPE_VENDOR_ROOT => [USER_TYPE_ADMIN_ROOT => false, UserTypes::ADMIN => false, USER_TYPE_VENDOR_ROOT => true,  UserTypes::VENDOR => true,  UserTypes::CUSTOMER => false],
        UserTypes::VENDOR     => [USER_TYPE_ADMIN_ROOT => false, UserTypes::ADMIN => false, USER_TYPE_VENDOR_ROOT => false, UserTypes::VENDOR => true,  UserTypes::CUSTOMER => false],
        UserTypes::CUSTOMER   => [USER_TYPE_ADMIN_ROOT => false, UserTypes::ADMIN => false, USER_TYPE_VENDOR_ROOT => false, UserTypes::VENDOR => false, UserTypes::CUSTOMER => false],
    ];

    $current_user_type = $target_user_type = UserTypes::CUSTOMER;

    if (UserTypes::isAdmin($current_user_info['user_type'])) {
        $current_user_type = YesNo::isTrue($current_user_info['is_root']) ? USER_TYPE_ADMIN_ROOT : UserTypes::ADMIN;
    } elseif (UserTypes::isVendor($current_user_info['user_type'])) {
        $current_user_type = YesNo::isTrue($current_user_info['is_root']) ? USER_TYPE_VENDOR_ROOT : UserTypes::VENDOR;
    }

    if (UserTypes::isAdmin($target_user_info['user_type'])) {
        $target_user_type = YesNo::isTrue($target_user_info['is_root']) ? USER_TYPE_ADMIN_ROOT : UserTypes::ADMIN;
    } elseif (UserTypes::isVendor($target_user_info['user_type'])) {
        $target_user_type = YesNo::isTrue($target_user_info['is_root']) ? USER_TYPE_VENDOR_ROOT : UserTypes::VENDOR;
    }

    return $login_as_map[$current_user_type][$target_user_type] ?? false;
}
 ...
 * @return string|void
 */
function fn_load_backoffice_color_scheme_variants(array $user_data)
{
    if (
        isset($user_data['user_id'], $user_data['user_type'])
        && ($user_data['user_type'] === UserTypes::ADMIN || $user_data['user_type'] === UserTypes::VENDOR)
    ) {
        $backoffice_color_scheme = fn_get_user_additional_data(BACKOFFICE_COLOR_SCHEME, $user_data['user_id']);
